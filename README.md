...
Graphs are static DAGs. Bind each step to either inline `agent.system` or a source-qualified `agent.ref`; model synthesis as a normal dependent step. Optional `steps[].agent.model` and `steps[].agent.thinking` pin the launch lane for one step. Precedence is step override, then library agent frontmatter, then parent defaults captured at `start`. Use `needs` for success-gated dependencies and `after` when a downstream step should consume terminal evidence from failed or blocked lanes too. Sink steps, not array order, define caller-facing finals.
