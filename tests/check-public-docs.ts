...
function checkPublicSurfaceOwnership(): void {
	...
	requireFragments("skills/pi-multiagent/references/graph-cookbook.md", cookbook, ["structured wait receipt", "accepted-for-delivery transport only", "fails before child launch if explicit callable `exa_search` and `exa_fetch` `extensionTools` are missing"]);
