...
test("materializeAgentTeamInput rejects non-start, mixed, and control-field graph files", async () => {
	...
	assert.equal(materializeAgentTeamInput({ action: "retrieve", runId: "agt_abcdefghijklmnopqrstuvwxyzABCDEF1234567890-_", graphFile: "graph.json" }, root).diagnostics.some((item) => item.code === "graph-file-start-only"), true);
