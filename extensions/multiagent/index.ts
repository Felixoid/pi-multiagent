...
export function registerMultiagentExtension(pi: ExtensionAPI, extensionOptions: MultiagentExtensionOptions = {}) {
	...
	pi.registerTool({
		...
		promptGuidelines: [
			...
			"Use graph.steps[].agent.system for inline agents or graph.steps[].agent.ref with source-qualified refs such as package:reviewer. Optional graph.steps[].agent.model and thinking pin one step's launch lane; precedence is step field, agent frontmatter, then parent start-time default.",
